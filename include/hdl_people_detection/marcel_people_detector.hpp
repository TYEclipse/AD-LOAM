...
public:
   ...
   */
  MarcelPeopleDetector(int min_pt, int max_pt, const Eigen::Array3f& min_size, const Eigen::Array3f& max_size,float tolerance,float lambda)
    : min_pt(min_pt),
      max_pt(max_pt),
      min_size(min_size),
      max_size(max_size),
   ...
   */
  std::vector<Cluster::Ptr> detect(const pcl::PointCloud<pcl::PointXYZINormal>::ConstPtr& cloud) {

    // project #cloud into XY-space
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr scaled(new pcl::PointCloud<pcl::PointXYZINormal>());
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> object_clouds;
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> detected;
    detected.reserve(candidate_clusters.size());
    for (uint i = 0; i < candidate_clusters.size(); i++) {
      ...
      //object_clouds.push_back(cluster_cloud);


      auto splitted = split(cluster_cloud, lambda);
      auto merged = merge(splitted);

      //      std::copy_if(merged.begin(), merged.end(), std::back_inserter(object_clouds), [&](const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& c) { return isValid(c); });
      std::copy(merged.begin(), merged.end(), std::back_inserter(object_clouds));
    }

    std::vector<Cluster::Ptr> clusters(object_clouds.size());
    std::transform(object_clouds.begin(), object_clouds.end(), clusters.begin(), [=](const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& cloud) { return std::make_shared<Cluster>(cloud); });

    return clusters;
  }

private:
  bool isValid(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& cloud) const {
  ...
  std::vector<pcl::PointIndices> clustering(const pcl::PointCloud<pcl::PointXYZINormal>::ConstPtr& cloud) const {
    pcl::search::KdTree<pcl::PointXYZINormal>::Ptr kdtree(new pcl::search::KdTree<pcl::PointXYZINormal>());
    kdtree->setInputCloud(cloud);

    pcl::EuclideanClusterExtraction<pcl::PointXYZINormal> extractor;
    extractor.setClusterTolerance(tolerance);
    extractor.setMinClusterSize(2);
    extractor.setMaxClusterSize(cloud->size());
  ...
  std::vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> split(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& cloud, float lambda) const {
    kkl::alg::DPmeans<float, 2> dp_means;

    std::vector<Eigen::Vector2f> points(cloud->size());
    std::transform(cloud->begin(), cloud->end(), points.begin(), [=](const pcl::PointXYZINormal& pt) { return pt.getVector3fMap().topLeftCorner(2, 1); });
    if (!dp_means.train(points, lambda, 0.02f, 128)) {
      // std::cerr << "warning : dp_means not converged!!" << std::endl;
    }

    std::vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> splitted(dp_means.centroids.size());
    for (uint i = 0; i < splitted.size(); i++) {
      splitted[i].reset(new pcl::PointCloud<pcl::PointXYZINormal>);
    }
    ...
    splitted.erase(std::remove_if(splitted.begin(), splitted.end(), [=](const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& c) { return c->empty(); }), splitted.end());

    return splitted;
  }

  // re-merge clusters by gap detection
  std::vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> merge(std::vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr>& clouds) const {
    ...
    for (uint i = 0; i < clouds.size(); i++) {
      for (uint j = i + 1; j < clouds.size(); j++) {
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> merged(clouds.size());
    for (uint i = 0; i < merge_to.size(); i++) {
      if (merge_to[i] < 0) {
        merged[i] = clouds[i];
        continue;
      }
    ...
    merged.erase(std::remove_if(merged.begin(), merged.end(), [=](const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& c) { return c == nullptr; }), merged.end());

    return merged;
  }

  // gap detection
  bool detectGap(const Eigen::Vector2f& mean1, const Eigen::Vector2f& mean2, const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& cloud1, const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& cloud2) const {
    Eigen::Vector2f major = (mean2 - mean1).normalized();

    std::vector<float> on_major(cloud1->size() + cloud2->size());
    std::transform(cloud1->begin(), cloud1->end(), on_major.begin(), [=](const pcl::PointXYZINormal& pt) { return major.dot(pt.getVector3fMap().topLeftCorner(2, 1)); });
    std::transform(cloud2->begin(), cloud2->end(), on_major.begin() + cloud1->size(), [=](const pcl::PointXYZINormal& pt) { return major.dot(pt.getVector3fMap().topLeftCorner(2, 1)); });
    ...
    for (uint i = 0; i < on_major.size(); i++) {
      uint n = (on_major[i] - min_val) / (max_val - min_val) * hist_size;
      if (n >= 0 && n < hist.size()) {
        hist[n] ++;
      }
    }
