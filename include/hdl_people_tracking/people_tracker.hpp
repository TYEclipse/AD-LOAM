...
#include <aloam_velodyne/Cluster.h>
 ...
 */
template<>
boost::optional<double> distance(const std::shared_ptr<hdl_people_tracking::KalmanTracker>& tracker, const aloam_velodyne::Cluster& observation) {
...
public:
  PeopleTracker(ros::NodeHandle& private_nh) {
    ...
    data_association.reset(new kkl::alg::NearestNeighborAssociation<KalmanTracker::Ptr, aloam_velodyne::Cluster>());
//    data_association.reset(new kkl::alg::GlobalNearestNeighborAssociation<KalmanTracker::Ptr, VisualDetection>());
   ...
   */
  void correct(const ros::Time& time, const std::vector<aloam_velodyne::Cluster>& detections) {
    // data association
...
public:
  ...
  std::unique_ptr<kkl::alg::DataAssociation<KalmanTracker::Ptr, aloam_velodyne::Cluster>> data_association;
};
